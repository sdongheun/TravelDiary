...
## 8.5 장소 기록 범위 정책 (확정 — 하이브리드)

- **기록·일기(비공개)**: **전 지역 허용**. TourAPI 주변 제안 → 맞으면 `place_id` 연결, 없거나 비관광지면 한글 검색(`searchKeyword2`)/직접 입력(`manual_location`).
- **공식정보 결합**: TourAPI 장소로 확정된 경우에만 개요·이미지 자동 표시.
- **후기 게시판(공개)**: **TourAPI 관광지에만** 작성 가능(같은 관광지 혼행 후기 묶음 = 관광 선순환).
- **장소 데이터 소스는 TourAPI 우선**(주변+키워드). 카카오는 지도 표시·마커 용도로만(비관광지를 카카오 POI로 과하게 채우지 말 것 — TourAPI 비중 유지).
- **지도 "주변 관광지 보기" 토글**(기본 OFF): ON 시 미방문 TourAPI 관광지를 `?` 핀으로 오버레이(탭 → 공식정보 + 기록 추가/후기 보기) → 발굴·재방문 유도. 클러터 제어(반경·카테고리·top-N·줌) 필수.

## 8.6 사진 캡처 · 입력 정책 (확정)

- **사진 다중 선택 → EXIF 촬영시각 시간순 정렬 → 사진별 페이지("다음" 흐름)**. EXIF는 압축 전 먼저 추출.
- **메모는 사진별(`photos.caption`, 신설 예정) · 선택·가벼움**.
- **같은 `place_id` 사진은 자동으로 1 record로 묶음**(그룹핑=장소 기준, 배열=시간순). 사용자는 필요 시 분리/수정.
- **AI 감정 질문은 장소(record) 단위 1회**(사진별 X) — 입력 최소화 + 일기 품질 양립. caption(사진별 디테일) + 질문 1개 + 평점 → `records.ai_summary`.
- `photos.caption` 추가는 Phase 3에서 신규 마이그레이션으로(append-only).
